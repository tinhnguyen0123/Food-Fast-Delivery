...
export default function RestaurantRegisterPage() {
  ...
  const handleGoToStep2 = () => {
    ...
    if (form.password.length < 3) {
      toast.error("Mật khẩu phải có tối thiểu 3 ký tự");
      return;
    }
    // Nếu tất cả đều hợp lệ, chuyển bước
    setCurrentStep(2);
  };

  // *** HÀM MỚI ĐỂ VALIDATE BƯỚC 2 ***
  const handleGoToStep3 = () => {
    if (!form.address) {
      toast.error("Vui lòng chọn vị trí hoặc nhập địa chỉ chi tiết");
      return;
    }
    // Nếu tất cả đều hợp lệ, chuyển bước
    setCurrentStep(3);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate (Logic này của bạn đã tốt, nó kiểm tra lại tất cả)
    if (!form.restaurantName || !form.email || !form.password) {
      toast.error("Vui lòng nhập đủ tên nhà hàng, email và mật khẩu");
      setCurrentStep(1); // Quay lại bước 1 nếu thiếu
      return;
    }
    if (!form.address) {
      toast.error("Vui lòng chọn địa chỉ trên bản đồ hoặc nhập địa chỉ");
      setCurrentStep(2); // Quay lại bước 2 nếu thiếu
      return;
    }
    if (!imageFile) {
      toast.error("Vui lòng chọn ảnh đại diện nhà hàng");
      // Đã ở bước 3, không cần chuyển
      return;
    }
            ...
            {currentStep === 1 && (
              <div className="p-8">
                ...
                <div className="flex justify-between mt-8">
                  ...
                  <button
                    type="button"
                    onClick={handleGoToStep2} // *** THAY ĐỔI Ở ĐÂY ***
            ...
            {currentStep === 2 && (
              <div className="p-8">
                ...
                <div className="flex justify-between mt-8">
                  ...
                  <button
                    type="button"
                    onClick={handleGoToStep3} // *** THAY ĐỔI Ở ĐÂY ***
                    className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all"
                  >
                    Tiếp theo
                    <ArrowLeft className="w-5 h-5 rotate-180" />
